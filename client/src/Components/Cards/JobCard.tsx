...
const JobCard = ({ RefreshApplications, _id, company, jobTitle, expiry, requirements, reply, applicationLink }: { RefreshApplications: Function, _id: string, company: string, jobTitle: string, expiry: string, requirements: string, reply: string, applicationLink: string }) => {
    ...
    return (
        <>
            <div className="w-[400px] h-[189px] border border-black-500 rounded-[10px] flex flex-col justify-between px-[21px] py-[13px] relative linear-black">
                ...
                <button onClick={() => setShowReplyMenu(true)} className="self-start underline text-black-400">{reply ? reply.charAt(0).toUpperCase() + reply.slice(1) : "No reply specified"}</button>
                ...
                <div onMouseOver={() => setShowMenu(true)} className="bg-transparent absolute w-full h-[40%] top-[50%] translate-y-[-45%] left-0"></div>
                {
                    ...
                    showMenu &&
                    <div onMouseLeave={() => setShowMenu(false)} className="transition ease-in-out duration-500 opacity-100 hover:opacity-100 absolute glass w-full h-full top-0 left-0 rounded-[10px] flex flex-col gap-[8px] items-start p-[16px] fade-in-fast">
